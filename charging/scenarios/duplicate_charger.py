...
async def legit_client_runnable(cp: ChargePointClientBase):
    cp.print_message("Legit client is connected")


async def malicious_client_runnable(cp: ChargePointClientBase):
...
async def main():
    ...
    legit_client = asyncio.create_task(
        launch_client(**CONFIG, async_runnable=legit_client_runnable, printed_name='Legit')
    )
    ...
    malicious_client = asyncio.create_task(
        launch_client(**CONFIG, async_runnable=malicious_client_runnable, printed_name='Malicious')
    )
